...
if added:
    print(f"[warn] added tables: {added}")
if removed:
    print(f"[warn] removed tables: {removed}")
...
for t in common:
    ...
    status = []
    if dr != 0:
        status.append(f"rows {o['rows']}→{n['rows']} ({pct:+.2f}%)")
    if hash_changed:
        status.append("hash changed")
    if status:
